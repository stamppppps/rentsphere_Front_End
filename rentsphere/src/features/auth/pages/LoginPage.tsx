...
const LoginPage: React.FC = () => {
  ...
  const [identifier, setIdentifier] = useState(""); // email หรือ เบอร์ (แต่ backend ส่วนใหญ่ใช้ email)
  ...
  async function handleLogin() {
    try {
      ...
      // ✅ Backend ส่วนมากรับ email+password
      // ถ้า backend ของเธอรับ phone ได้ด้วย ค่อยปรับฝั่ง backend ให้รองรับ identifier
      const payload = {
      ...
      // ✅ redirect ตาม role จาก backend (ชัวร์สุด)
      ...
      <div className="flex-1 relative overflow-hidden flex items-center justify-center p-8">
        ...
        <div className={`relative z-10 ${cardSaaS}`}>
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            {/* แค่ช่วย debug ว่าอยู่ role ไหนจาก URL */}
            <p className="text-[10px] text-slate-500">
              Current role route: {userRole === UserRole.OWNER ? "OWNER" : "TENANT"}
            </p>
          </form>
        </div>
      </div>
