...
export default function BillingTable({ data, onSelect }: BillingTableProps) {
  ...
  const pageData = useMemo(() => {
    return data.slice((currentPage - 1) * PER_PAGE, currentPage * PER_PAGE);
  }, [data, currentPage]);

  const startIdx = data.length > 0 ? (currentPage - 1) * PER_PAGE + 1 : 0;
  const endIdx = Math.min(currentPage * PER_PAGE, data.length);
