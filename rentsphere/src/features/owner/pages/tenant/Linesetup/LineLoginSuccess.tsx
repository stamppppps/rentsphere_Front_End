    ...
    useEffect(() => {
        const run = async () => {
            try {
                ...
                const lineUserId = params.get("lineUserId") || localStorage.getItem("lineUserId") || "";

                if (!lineUserId) {
                    setStatus("error");
                    setMsg("ไม่พบ lineUserId — กรุณาเข้าสู่ระบบ LINE ใหม่");
                    return;
                }
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-indigo-100 p-8 w-full max-w-lg">
                ...
                {status === "error" && (
                    <div className="text-rose-600 font-bold">
                        ...
                        <div className="mt-4">
                            <button
                                onClick={() => (window.location.href = "/owner/line-login")}
