...
export default function AdminParcel() {
  ...
  // ===== Send tab state (UI only) =====
  const [tenants] = useState<Tenant[]>([]); // ไม่มี mock, รอเสียบ backend ใหม่
  ...
  // ===== History tab state (UI only) =====
  const [history] = useState<ParcelRecord[]>([]); // ไม่มี mock, รอเสียบ backend ใหม่
  ...
  const filteredHistory = useMemo(() => {
    ...
    return list.filter((h) => {
      const hay = `${h.tenantName || ""} ${h.room || ""} ${h.note || ""} ${
        h.status || ""
      }`.toLowerCase();
  ...
  const submit = () => {
    ...
    // UI only
    setOk("UI Only ✅ (ตอนนี้ยังไม่เรียก backend / ยังไม่ส่ง LINE)");
  };

  const refreshHistoryUI = () => {
    resetMessages();
    setOk("UI Only ✅ (ตอนนี้ยังไม่ดึงประวัติจาก backend)");
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs font-semibold text-slate-500">
                {activeTab === "send"
                  ? "อัปโหลดรูป + (รอเสียบ backend ใหม่เพื่อส่ง LINE)"
                  : "ดูรายการที่เคยส่งย้อนหลัง (รอเสียบ backend ใหม่)"}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-indigo-100 shadow-xl shadow-indigo-100/30 overflow-hidden">
            <div className="p-6 md:p-8">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-3 md:gap-4">
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-xs text-slate-500 font-semibold">
                    * ตอนนี้ยังไม่โหลดผู้เช่าจาก backend (UI only)
                  </div>
                </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-black text-slate-900">
                          ลากรูปมาวาง หรือกดเพื่อเลือกไฟล์
                        </div>
                        <div className="text-xs font-semibold text-slate-500">
                          (UI Only) ตอนนี้ยังไม่ส่ง LINE
                        </div>
                      </div>
                    </div>
                  ) : (
              ...
              <div className="mt-7">
                ...
                >
                  ส่งแจ้งพัสดุ + ส่ง LINE
                </button>

                <div className="mt-3 text-xs text-slate-500 font-semibold">
                  * (UI Only) ปุ่มนี้ยังไม่เรียก backend / ยังไม่ส่ง LINE
                </div>
              </div>
            </div>
            ...
            <div className="p-6 md:p-8">
              <div className="flex flex-col md:flex-row md:items-end gap-3 md:gap-4">
                ...
                <div className="md:w-40">
                  <button
                    onClick={refreshHistoryUI}
                    className="w-full px-4 py-3 rounded-2xl bg-indigo-50 hover:bg-indigo-100 font-black text-indigo-700 transition"
                  >
                    รีเฟรช
                  </button>
                </div>
                ...
                {filteredHistory.length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <div className="text-slate-700 font-black">
                      ยังไม่มีข้อมูลประวัติ (UI Only)
                    </div>
                    <div className="text-xs font-semibold text-slate-500 mt-1">
                      ยังไม่ได้เชื่อม backend ใหม่
                    </div>
                  </div>
                ) : (
