...
const ApproveRejectModal: React.FC<ApproveRejectModalProps> = ({
  ...
  const [reason, setReason] = useState(
    isApprove ? `ข้อมูลครบถ้วน ${config.actionLabel}` : ""
  );
      ...
      <div className="bg-white w-full max-w-lg rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-500">
        ...
        <div className="p-10 text-center">
          <div className="relative inline-block mb-8">
            <div
              className={`w-24 h-24 rounded-[32px] flex items-center justify-center shadow-2xl transition-all duration-500 ${
                isApprove
            ...
            <div
              className={`absolute -bottom-2 -right-2 w-10 h-10 rounded-2xl flex items-center justify-center text-white border-4 border-white shadow-lg ${
          ...
          <p className="text-slate-500 font-medium leading-relaxed mb-8">
            ระบุเหตุผลในการ{config.label}การจองของคุณ{" "}
            <span className="text-slate-900 font-bold">{bookingName}</span>
          </p>
          ...
          <div className="space-y-3 text-left mb-8">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-2">
              <MessageSquare
                size={12}
                className={isApprove ? "text-emerald-500" : "text-rose-500"}
              />
