import { ArrowRight, CheckCircle2, Clock, Hammer, Users } from "lucide-react";
...
const FacilityCard: React.FC<FacilityCardProps> = ({ facility }) => {
  ...
  const maxPeople = facility.bookingSetting?.maxPeople ?? 0;
  ...
  return (
    <div
      className={`bg-white rounded-[32px] overflow-hidden border transition-all duration-300 group flex flex-col h-full ${isAvailable
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex gap-2">
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider backdrop-blur-md shadow-sm border ${isAvailable
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="mb-4">
          ...
          <p className="text-sm text-slate-400 font-medium line-clamp-2 leading-relaxed h-10">
            {facility.description || "ไม่มีรายละเอียด"}
          </p>
        </div>
        ...
        <div className="mt-auto">
          <Link
            ...
            className={`w-full flex items-center justify-center gap-2 py-3.5 px-4 rounded-2xl font-black transition-all duration-300 shadow-xl active:scale-95 ${isAvailable
