...
type FeatureType = "parcel" | "billing" | "maintenance" | "booking";
...
const LatestActivitySection: React.FC<LatestActivitySectionProps> = ({
  activities,
  onSeeAll,
}) => {
  ...
  return (
    <div className="px-6 mt-8 mb-6">
      <div className="flex justify-between items-center mb-4">
        ...
        <button
          onClick={onSeeAll ? onSeeAll : () => navigate("/tenant/history")}
        ...
        {activities.map((activity) => {
          ...
          return (
            <div
              ...
              onClick={activity.onClick ? activity.onClick : () => navigate("/tenant/history")}
            >
              <div
                className={`w-11 h-11 ${bg} ${color} rounded-xl flex items-center justify-center transition-transform group-hover:scale-105`}
              >
                {icon}
              </div>
